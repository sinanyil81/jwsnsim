...
public class FtspNodeLSAverage extends Node implements TimerHandler{
	...
	public FtspNodeLSAverage(int id, Position position) {
		super(id,position);
	...
	void adjustLine2(UInt32 localTime){
        if(is_synced()){
        	...
        	if(numLines >= 4){
        		/* throw out the smallest and the biggest elements */
        		for(int i= 1; i < numLines-1; i++){
            		slopeAvg += slopeTable[i]/(float)(numLines-2);
            	}	
        	}
        	else{
            	for(int i= 0; i < numLines; i++){
	...
	void adjustLine3(UInt32 localTime){
        if(is_synced()){
